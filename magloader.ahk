...
IfMsgBox Yes 
{
	MsgBox % "AutoLoader allows up to four kinds of bullets to be loaded into a magazine at a time, with any number of bullets in each group. What it does then is split up the bullets into sperate stacks, with how many it grabs out of each stack being up to the user. `n `n `t I recommend you make a bit of space in the Stash compartment of your Character before actually starting the script. Once you close this window, a new window will ask for the following info: how many bullets do you want the script to take out of the stack at a time, how many kinds of bullets you will be using, and how many times you want to auto-load in total. `n `n `t Input how many bullets you want to take out of a stack at a time, and if you have less than 4 kinds of bullets for it, simply leave the remaining groups blank. For example, if you want to put two bullets of the same kind into a 30-round mag, enter 2 into that group. The script will count the amount of rounds loaded, to the total number of rounds or in our case 30."
}else{}
